...
OUTPUT_PNG             = "isolines_p80000_k10_mad.png"
...
# ---------- utility: invert depth to negative height ----------
def invert_depth(df: pd.DataFrame) -> pd.DataFrame:
    """
    Преобразует глубины в отрицательные значения высоты:
    Z := -Z
    Делает копию df, чтобы не ломать оригинал.
    """
    df = df.copy()
    df['Z'] = -df['Z']
    return df
...
def grid_and_sanitize(points: pd.DataFrame,
    ...
    # локальный σ 3×3 и «умный» медианный фильтр
    def win_std(a):
        if np.isnan(a).all():
            return 0.0
        return np.nanstd(a)
...
# ---------------------------------
# ---------- plotting ----------
def plot_3d_grid(xi: np.ndarray, yi: np.ndarray, zi: np.ndarray, out_png: str = None):
    """
    Строит 3D поверхность по регулярной сетке.
    xi, yi, zi: выходы grid_and_sanitize
    """

    fig = plt.figure(figsize=(10, 7))
    ax  = fig.add_subplot(111, projection='3d')
    # note: rstride, cstride — шаг сетки для ускорения отрисовки
    surf = ax.plot_surface(xi, yi, zi,
                           rstride=1, cstride=1,
                           edgecolor='none',       # гладкая заливка
                           antialiased=True)
...
def main():
    ...
    plot_3d_grid(xi, yi, zi, out_png="3d_grid.png")
