...
def render_runoff_temp(era5_df: pd.DataFrame):
    with st.expander("Runoff vs Temperature Trends", expanded=False):
        ...
        plot_run_df = run_tr_df.resample("ME").mean() if agg_r == "Monthly" else run_tr_df
        st.line_chart(plot_run_df)
        max_lag_r = st.slider("Max lag for temp→runoff corr (days)", 5, 120, 45, 5, key="runoff_temp_lag")
        ...
        fig_run_sc = px.scatter(rb_scatter, x="t2m_c", y="runoff_mm", color="year", trendline=None, title="Temperature vs Runoff")
