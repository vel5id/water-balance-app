...
try:
    ...
    def make_trend_comparison_figure(p_series, et_series, p_slope, p_inter, et_slope, et_inter):
        ...
        return fig
from wbm.ensemble import run_volume_ensemble
...
if p_clim.empty or et_clim.empty:
    ...
    if forecast_mode == "Monthly mean (all years)":
        ...
        hwd = int(hist_window_days) if hist_window_days and hist_window_days > 0 else None
        p_daily = build_robust_season_trend_series(
            era5_df, "date", "precip_mm",
            pd.Timestamp(start_date), end_date,
            history_window_days=hwd,
            season_basis=seas_basis, seasonal_smooth_window=seas_smooth,
            center=center_mode,
        )
        et_daily = build_robust_season_trend_series(
            era5_df, "date", "evap_mm",
            pd.Timestamp(start_date), end_date,
            history_window_days=hwd,
            season_basis=seas_basis, seasonal_smooth_window=seas_smooth,
            center=center_mode,
    ...
    with st.expander("Ensemble forecast (experimental)", expanded=False):
        ...
        if use_ens:
            ...
            block_len = st.slider("Residual block length (days)", 1, 14, 5, 1)
            q_low, q_med, q_high = st.select_slider(
                "Quantiles",
                options=[0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95],
                value=(0.05, 0.5, 0.95),
            )
            ens = run_volume_ensemble(
                start_date=pd.Timestamp(start_date), end_date=end_date,
                init_volume_mcm=init_volume,
                p_clim=p_clim, et_clim=et_clim, vol_to_area=vol_to_area,
                imerg_df=era5_df.rename(columns={"precip_mm": "precipitation_mm"}),
                gleam_df=era5_df.rename(columns={"evap_mm": "evaporation_mm"}),
                p_scale=p_scale, et_scale=et_scale, q_in_mcm_per_day=q_in, q_out_mcm_per_day=q_out,
                N=int(N), history_window_days=(None if int(hist_days) == 0 else int(hist_days)),
                season_basis=seas_basis, seasonal_smooth_window=int(seas_smooth), center=center_mode,
                use_residual_bootstrap=use_boot, residual_block_days=int(block_len), seed=42,
                quantiles=(float(q_low), float(q_med), float(q_high)),
            )
            if not ens.empty:
                # Overlay fan chart by filling between quantiles using Plotly via st.plotly_chart again
