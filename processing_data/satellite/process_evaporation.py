"""Process ERA5 total evaporation (e) files into daily evaporation (positive mm/day).

ERA5 'e' is cumulative negative (downward) flux in meters of water equivalent. We invert sign and convert to mm.

Outputs:
  era5_evap_mm.csv
  era5_evap_mm.sqlite (table era5_evap_mm)
"""
...
ALIASES=['e','evaporation','total_evaporation']
TABLE_NAME='era5_evap_mm'
CSV_NAME=TABLE_NAME+'.csv'
DB_NAME=TABLE_NAME+'.sqlite'
AGG_MODE='sum'
CONV_MODE='M_TO_MM_NEG'
...
def open_inner_dataset(path: Path) -> xr.Dataset:
    ...
    if sig.startswith(b'PK'):
        ...
        with zipfile.ZipFile(path,'r') as zf:
            ...
            if not nc_members:
                raise RuntimeError('Zip without nc')
            inner=nc_members[0]
...
def save_csv(series: pd.Series, path: Path):
    ...
    pd.DataFrame({'date': series.index.strftime('%Y-%m-%d'), 'value': series.values}).to_csv(path, index=False)
...
def main():
    ap=argparse.ArgumentParser(description='Process ERA5 evaporation to daily mm (positive).')
