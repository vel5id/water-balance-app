...
# Список дат, по которым есть Sentinel-2 снимки (замени на свои даты)
sentinel_dates = [
    '2020-03-27',
    '2020-04-21',
    '2020-04-26',
...
# Параметры временного окна (например, ±1 день от даты Sentinel-2)
time_window_days = 7

# Функция для получения ближайшего изображения из коллекции (IMERG или MOD16)
...
# Загружаем коллекции IMERG и MOD16
imerg_ic = ee.ImageCollection('NASA/GPM_L3/IMERG_V06').select('precipitationCal')
mod16_ic = ee.ImageCollection('MODIS/006/MOD16A2').select('ET').map(lambda img: img.multiply(0.1))  # масштабирование ET

# Экспорт по датам списка
...
# Экспорт осадков IMERG (~10km)
export_by_dates(sentinel_dates, 'IMERG_precip', imerg_ic, 10000)
